...
Purpose: Defines typed response payloads returned by the Headstart run and chat workflows.
...
- 2026-03-02: Added follow-up chat completion response schema. (Codex)
Preconditions:
- Pydantic BaseModel is available.
Inputs:
- Acceptable: Non-empty markdown guide/assistant response strings.
- Unacceptable: Missing or non-string message fields.
Postconditions:
- Response objects can be validated for contract-compliant output.
Returns:
- `RunAgentResponse` and `ChatCompletionResponse` model instances.
Errors/Exceptions:
...
class ChatCompletionResponse(BaseModel):
    assistantMessage: str = Field(
        description="Single markdown/plaintext assistant response for a follow-up chat question."
    )
